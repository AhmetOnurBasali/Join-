async function onSubmit(event) {
  ...
  if (user) {
    ...
    window.location.href = "forgotPassword.html?msg=email is not registered here";
  }
}
...
async function tryToSendEmail(response) {
  ...
  setTimeout(() => {
    window.location.href = `forgotPassword.html?msg=${ response.ok? `email was send to ${email.value}`: "email not send(error: getEmail.js - tryToSendEmail(response)"}`;
  }, 1000);
}
